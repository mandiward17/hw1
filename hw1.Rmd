...
q1 <- df %>% 
  ...
  summarise(mean = mean(real_cost),
            sd = sd(real_cost),
            se = sd / sqrt(n()))  # double check this is 34; prob another way to do

q1 %>% 
  filter(country_name != "NA") %>% 
  mutate(country_name = fct_reorder(country_name, mean)) %>% 
  ggplot(aes(mean, country_name)) +
  geom_linerange(aes(xmin = mean - se, xmax = mean + se), color = "grey40") +
  geom_point(color = "cornflowerblue") +
  scale_x_continuous(labels = comma, limits = c(0, 33000), expand = c(0, 0)) +
  theme_minimal() +
