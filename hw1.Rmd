...
## Aspect ratio should be smaller on x-axis
```
...
```{r}
df1 <- df  %>%
   group_by(country_name, city, length, real_cost, stations)  #created new dataset with select varaibles 

df2 <- df1 %>% 
   summarise(mean_length = mean(length), 
             mean_realcost = mean(real_cost))  #computed mean of varaibles

df2 %>% 
ggplot(aes(mean_length, mean_realcost)) +
  geom_point(color = "gray80") +
  geom_point(data = filter(df2, str_detect(country_name, "India")),
               color = "purple") +
  #scale_color_manual("Number of transit systems", values = c("purple")) +
  scale_x_log10() +
  #ylim(c(30000, 90000)) +
  ...
  #gghighlight(mean_length > 5 | mean_length < 100) +
...
##currently working on getting correct country labels, fixing y axis scale, getting the legend to show up, and to highlight correct countries. 
