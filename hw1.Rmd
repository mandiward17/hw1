...
to_label_1 <- c("Boston", "Los Angelas", "New York", "San Francisco", "San Jose", "Seattle")
...
df2 %>% 
ggplot(aes(mean_length, mean_realcost)) +
  geom_mark_ellipse(aes(group = city, label = "United States"),
                    data = filter(subset_cities_2)) +
                    #country == "United States")) + 
  geom_point(aes(size = n), color = "gray80") +
  geom_point(
    aes(size = n),
    data = filter(df2, str_detect(country, "US")),
    color = "#2d81ae"
  ) +
